...
function safeErrorMessage(data, fallback = 'Unknown OpenAI error') {
...
export default async function handler(req, res) {
  ...
  if (!process.env.OPENAI_API_KEY) {
    return res.status(200).json({
      ok: true,
      liveAI: false,
      package: fallbackPackage(seed),
      warning: 'OPENAI_API_KEY is missing, returned safe fallback package.'
    });
  }
  ...
  try {
    ...
    if (!response.ok) {
      return res.status(200).json({
        ...
        liveAI: false,
        package: fallbackPackage(seed),
        warning: 'OpenAI call failed, returned safe fallback package.',
        openaiStatus: response.status,
        openaiMessage: safeErrorMessage(data)
      });
    }
    ...
    return res.status(200).json({ ok: true, liveAI: true, package: parsed });
  } catch (error) {
    return res.status(200).json({
      ok: true,
      liveAI: false,
      package: fallbackPackage(seed),
      warning: 'OpenAI request crashed, returned safe fallback package.',
